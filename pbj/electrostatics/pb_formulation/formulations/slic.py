...
def solve_sigma(self):
    ...
    x, info = scipy.sparse.linalg.gmres(
        A_op,
        b_vec,
        x0=self.slic_sigma.coefficients,
        tol=self.gmres_tolerance,
        maxiter=self.gmres_max_iterations,
        callback=callback,
    )
    ...
    sigma = bempp.api.GridFunction(slp_in_diel.domain, coefficients=x.ravel())
...
def calculate_potential(simulation, rerun_all, rerun_rhs):
    for index, solute in enumerate(simulation.solutes):
        ...
        solute.slic_sigma = bempp.api.GridFunction(
            dirichl_space_diel,
            coefficients=np.zeros(dirichl_space_diel.global_dof_count),
        )
    ...
    for index, solute in enumerate(simulation.solutes):
        matrix_solute = [
            solute.matrices["A"][1, 1] * (solute.ep_stern / solute.ep_in),
        ...
        for index_j, solute_j in enumerate(simulation.solutes):
            if index_j != index:
                ...
                inter_matrix_solute.append(
                    solute.matrices["A_inter"][index_array][3, 3]
    ...
    while it < max_iterations and phi_L2error > tolerance:
        ...
        for index, solute in enumerate(simulation.solutes):
            ...
            for index_j, solute_j in enumerate(simulation.solutes):
                if index_j != index:
                    ...
                    A_inter[3, 3] = (
                        inter_matrix_cache[index][index_array]
                        * solute_j.slic_e_hat_stern
                    )
        ...
        phi_L2error = np.sqrt(np.sum((phi_old - phi_new) ** 2) / np.sum(phi_new**2))

        phi_old = phi_new.copy()

        bempp.api.log(
            "PBJ: Self-consistent iteration %i, residual %e" % (it, phi_L2error)
        )

        it += 1

        for index, solute in enumerate(simulation.solutes):
            time_matrix_initialisation.append(
                solute.timings["time_matrix_initialisation"]
            )
...
def calculate_potential_slic(simulation):
    ...
    for index, solute in enumerate(simulation.solutes):
        ...
        if simulation.formulation_object.invert_potential:
            solute.results["d_phi_stern"] = (solute.ep_ex / solute.ep_stern) * solution[
                3
            ]
        else:
